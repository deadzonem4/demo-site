...
const SingleLeagueEvents = props => {
  
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  const single = props.data;
  ...
  return(
    <Link key={single.id} className="single-event-box" to={`/event/${single.id}`}>
      <div className="event-info">
        { (single.event_status.type === "finished" || single.event_status.code === "finished_aet") ?
          (
            <span className="event-status">КР<div className="events-right-line"></div></span>
          ) : 
          (single.event_status.type === "cancelled" || single.event_status.code === "postponed") ?
          (
            <span className="event-status">ОТЛ<div className="events-right-line"></div></span>
          ) :
          (
            (single.minute) ? 
            (
            ...
            (
              ...
              (
                <span className="event-status"><EventsDate date={single.start_time} customClass="events-time full-date" /><HourDate date={single.start_time} customClass="events-time" /><div className="events-right-line"></div></span>
              )
            )
          )
        }
...
export default SingleLeagueEvents;
